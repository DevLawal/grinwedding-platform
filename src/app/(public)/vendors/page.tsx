            ...
            <div className="container">
                ...
                <header className="mb-20 border-b border-editorial pb-16 max-w-4xl">
                    ...
                    <p className="text-xl text-text-muted leading-relaxed font-medium">
                        ...
                        <br />
                        <span className="text-[10px] font-black text-text-dim uppercase tracking-[0.4em] mt-8 block">
                    ...
                    {['Market All', 'Venues', 'Photography', 'Florists', 'Planners'].map((filter) => (
                        <button
                            ...
                            className={`text-[10px] font-black uppercase tracking-[0.3em] transition-colors pb-4 ${filter === 'Market All'
                                ? 'text-text border-b-2 border-purple'
                        ...
                        <tbody className="divide-y divide-editorial">
                            {vendors.map((vendor, index) => (
                                <tr key={vendor.id} className="group hover:bg-surface-2 transition-all duration-300">
                                    <td className="py-8 px-8 font-mono text-text-dim/30 font-black text-lg group-hover:text-text transition-colors">
                                    ...
                                    <td className="py-8 px-8">
                                        <div className="flex items-center gap-6">
                                            ...
                                            <div>
                                                <Link href={`/vendors/${vendor.slug}`} className="block text-[0.9rem] font-black text-text hover:text-purple transition-all uppercase tracking-widest mb-1 leading-tight">
                                                    {vendor.name}
                                                </Link>
                                    ...
                                    <td className="py-8 px-8 text-center">
                                        <div className="inline-flex flex-col items-center">
                                            <span className="text-lg font-mono font-black text-text">{(vendor.rating || 0).toFixed(1)}</span>
                                            <span className="text-[8px] uppercase text-text-dim font-black tracking-[0.2em]">Scale / 5.0</span>
                                        </div>
                                    </td>
                                    <td className="py-8 px-8 text-text-dim text-[10px] font-black uppercase tracking-widest">
                                        {vendor.location}
                                    </td>
                                    <td className="py-8 px-8 text-right font-mono text-[10px] font-black text-text-dim uppercase tracking-widest">
                ...
                {/* Footer Disclaimer */}
                <div className="mt-12 text-[9px] font-black text-text-dim uppercase tracking-[0.3em] text-center">
